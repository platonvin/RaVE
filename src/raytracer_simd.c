...
static rave_pack pack(rave_vec3 v1, rave_vec3 v2){
    rave_pack packed = {};
    packed.repr.v1.x = v1.x;
    packed.repr.v1.y = v1.y;
    packed.repr.v1.z = v1.z;

    packed.repr.v2.x = v2.x;
    packed.repr.v2.y = v2.y;
    packed.repr.v2.z = v2.z;   

    return packed;
}

static rave_vec3 Random3D() {
    float a,b,c;
    a = Random1D();
    b = Random1D();
    c = Random1D();
...
//is actually precise
rave_ivec2 CastRay_precise_simd(rave_ctx* ctx, rave_pack* rayOrigins, rave_pack* rayDirections, 
        float* fractions, rave_pack* normals, Material* materials){
    ...
    (*normals).x = -(fsteps.x * fcurrentStepDiretion.x);
    (*normals).y = -(fsteps.y * fcurrentStepDiretion.y);
    (*normals).z = -(fsteps.z * fcurrentStepDiretion.z);
...
static rave_pack trace_ray_simd(rave_ctx* ctx, rave_pack origins, rave_pack directions){
    ...
    rave_pack lights = {0};
    rave_pack reflections = {1,1,1,1,1,1,1,1};
// println
    rave_ivec2 hits = {1,1};
    for (int i = 0; (i < ctx->max_reflections); i++){
        if(hits.x && hits.y){
            hits = CastRay_precise_simd(ctx, &origins, &directions, &fractions, &normals, &materials);
        } else {
            if(hits.x) {
                hits.x = CastRay_precise(ctx, 
                    &origins.repr.v1, 
                    &directions.repr.v1, 
                    &fractions[0], 
                    &normals.repr.v1, 
...
static void dispatch_ray_simd(rave_ctx* ctx, int local_x, int local_y, int local_z, int ray_count){
    rave_pack origins = pack(
        ctx->rave_get_ray_pos(local_x+0, local_y, local_z),
        ctx->rave_get_ray_pos(local_x+1, local_y, local_z)
    );

    rave_pack directions = pack(
        ctx->rave_get_ray_dir(local_x+0, local_y, local_z),
        ctx->rave_get_ray_dir(local_x+1, local_y, local_z)
    );
    // counter++;
    // random_storage = (rave_vec2){(local_x*counter)*3.23, (local_y*counter)*6.432};

    rave_pack lights = trace_ray_simd(ctx, origins, directions);

    ctx->rave_store_light(local_x+0,local_y,local_z, (rave_vec3){lights.repr.v1.x, lights.repr.v1.y, lights.repr.v1.z});
    ctx->rave_store_light(local_x+1,local_y,local_z, (rave_vec3){lights.repr.v2.x, lights.repr.v2.y, lights.repr.v2.z});
}
void rave_dispatch(rave_ctx* ctx, int count_x, int count_y, int count_z){
    //for now single core non-simd
    srand(666);

    //scaled x = (2,1,1) "workgroup size"
    for(int x=0; x<(count_x+1)/2; x++){ 
        for(int y=0; y<count_y; y++){
            for(int z=0; z<count_z; z++){
                dispatch_ray_simd(ctx, x, y, z, sizeof(__m256)/sizeof(rave_vec4));
            }
        }
    }
}
