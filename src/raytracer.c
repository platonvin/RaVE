...
rave_vec4 Random3D() {
    ...
    b = Random1D();
    c = Random1D();
    return (rave_vec4){a,b,c};
}

rave_vec4 randomSpherePoint(rave_vec4 rand) {
    ...
    float z = u;
    return (rave_vec4){x, y, z};
}

rave_vec4 NormalOrientedHemispherePoint(rave_vec4 rand, rave_vec4 n){
    rave_vec4 v = randomSpherePoint(rand);
    float dot = 
          v.x * n.x +
          v.y * n.y +
          v.z * n.z;
...
bool initTvals(rave_vec4* tMax, rave_vec4* tDelta, rave_ivec3* blockPos, rave_vec4 rayOrigin, rave_vec4 rayDirection){
    rave_vec4 effective_origin = rayOrigin;

    rave_vec4 block_corner1;
...
//is actually precise
bool CastRay_precise(rave_ctx* ctx, rave_vec4 rayOrigin, rave_vec4 rayDirection, 
    ...
    steps.x = (rayDirection.x > 0);
    steps.y = (rayDirection.y > 0);
    steps.z = (rayDirection.z > 0);
    // steps.vec = _mm_castps_si128(_mm_cmpgt_ps(rayDirection.vec, _mm_set1_ps(0)));

    steps.x = 2 * steps.x - 1;
    steps.y = 2 * steps.y - 1;
    steps.z = 2 * steps.z - 1;
    // steps.vec = _mm_mul_epi32(steps.vec, _mm_set1_epi32(2));
    // steps.vec = _mm_sub_epi32(steps.vec, _mm_set1_epi32(1));
...
rave_vec4 reflect(rave_vec4 I, rave_vec4 N) {
...
void ProcessHit(rave_vec4* origin, rave_vec4* direction, 
                float fraction, rave_vec4 normal, Material material, 
            ...
            rave_vec4 hemisphereDistributedDirection = NormalOrientedHemispherePoint(Random3D(), normal);
...
// println
    rave_vec3 _origin = ctx->rave_get_ray_pos(local_x, local_y, local_z);
    rave_vec4 origin = {
        _origin.x,
        _origin.y,
        _origin.z,
    };
    rave_vec3 _direction = ctx->rave_get_ray_dir(local_x, local_y, local_z);
    rave_vec4 direction = {
        _direction.x,
        _direction.y,
        _direction.z,
...
// println
    ctx->rave_store_light(local_x,local_y,local_z, 
